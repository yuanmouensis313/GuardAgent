...
function approvalResult(decision: GuardDecision, client: GuardClient, params?: Record<string, unknown>) {
  return {
    ...
    requireApproval: {
      ...
      severity: decision.risk === "critical" ? "critical" as const : decision.risk === "high" ? "warning" as const : "info" as const,
      timeoutMs: 60_000,
      allowedDecisions: ["allow-once", "deny"] as Array<"allow-once" | "deny">,
      pluginId: "guard-openclaw",
      onResolution(resolution: "allow-once" | "allow-always" | "deny" | "timeout" | "cancelled") {
...
function emergencyResult(event: HookEvent, config: GuardConfig, outbound = false) {
  ...
  if (fallback.kind === "allow") return undefined;
  if (fallback.kind === "deny") return { block: true, blockReason: fallback.reason };
  return {
    requireApproval: {
...
function lifecycle(event: HookEvent, ctx: Record<string, unknown> | undefined, eventType: string, config: GuardConfig): void {
  try {
...
export default definePluginEntry({
  ...
  register(api) {
    ...
    api.on("reply_payload_sending", (event: HookEvent, ctx?: Record<string, unknown>) =>
      outboundDecision(event, ctx, event.payload ?? event, config),
      { priority: 10_000, timeoutMs: 500 },
    );
    ...
    api.on("before_agent_run", async (event: HookEvent, ctx?: Record<string, unknown>) => {
      ...
      if (/(ignore|disregard).{0,40}(previous|system).{0,100}(tool|shell|credential|policy)/is.test(text)) {
        ...
        try {
          const decision = await client.decide(normalized);
          if (decision.decision === "OBSERVE" || decision.decision === "ALLOW") return undefined;
          return {
            outcome: "block" as const, reason: decision.reason,
          ...
          return {
            outcome: "block" as const, reason: "guardd unavailable during input-injection review",
    ...
    api.on("before_install", async (event: HookEvent, ctx?: Record<string, unknown>) => {
      ...
      try {
        const decision = await client.decide(normalized);
        if (decision.decision === "DENY" || decision.risk === "critical") return { block: true, blockReason: decision.reason };
        return undefined;
      } catch {
