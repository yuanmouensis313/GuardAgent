...
function inside(candidate, workspace) {
    const absolute = resolve(workspace, candidate);
    const rel = relative(resolve(workspace), absolute);
    return rel === "" || (!rel.startsWith("..") && !isAbsolute(rel));
}
export function emergencyDecision(event, config, outbound = false) {
    const tool = String(event.toolName ?? "unknown").toLowerCase();
    const values = strings(event.params ?? {});
    const joined = values.join(" ");
    if (outbound || EXFIL.test(tool))
        return { kind: "deny", reason: "guardd unavailable: outbound action failed closed", risk: "critical" };
    if (SECURITY.test(joined))
        return { kind: "deny", reason: "guardd unavailable: security configuration is protected", risk: "critical" };
    if (DYNAMIC.test(joined))
        return { kind: "deny", reason: "guardd unavailable: dynamic execution failed closed", risk: "critical" };
