...
const byteLength = (value) => Buffer.byteLength(typeof value === "string" ? value : JSON.stringify(value) ?? String(value), "utf8");
...
export function sanitizeValue(input, maxScanBytes = SANITIZATION_MAX_SCAN_BYTES) {
    const transformations = [];
    let counter = 0;
    let truncated = false;
    const marker = (kind, length) => `<GUARD_REDACTED type="${kind}" length="${length}" ref="event-local-${++counter}">`;
        ...
        if (Array.isArray(record.content)) {
            record.content = record.content.map((block) => {
                ...
                if (typeof copy.text === "string")
                    copy.text = tagText(copy.text, toolName);
                return copy;
            });
        }
        if (typeof record.text === "string")
            record.text = tagText(record.text, toolName);
    }
    result.result_size = byteLength(result.value);
    return result;
}
import { GENERATED_SECRET_PATTERNS, SANITIZATION_MAX_SCAN_BYTES, SENSITIVE_FIELD_TOKENS, } from "./generated-sanitization-patterns.js";
