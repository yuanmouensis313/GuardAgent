  ...
  return <>
    ...
    {selected && <Drawer title="审批详情" onClose={() => selectApproval()}>{detail.isLoading ? <Loading /> : detail.error ? <ErrorState error={detail.error} /> : (() => {
      ...
      return <div className="detail-stack"><div className="detail-hero"><Badge value={item.risk} /><Badge value={item.status} /><code>{short(item.approval_id, 18)}</code></div>
        <KeyValue items={[["原因", item.reason], ["规则", item.rule_ids.map((x: string) => <code key={x}>{x} </code>)], ["复核门", item.resolution_gate || "none"], ["阻断复核", item.blocking_review_id ? <code>{item.blocking_review_id}</code> : "—"], ["服务端时间", formatTime(detail.data.server_time)], ["到期", formatTime(item.expires_at)], ["参数摘要", <code>{item.parameter_digest}</code>], ["操作者", item.operator || "尚未处理"]]} />
        ...
        {item.status === "pending" && item.resolution_gate === "blocked_by_review" && <div className="notice danger"><strong>需要独立安全覆盖</strong><span>模型高置信拒绝已阻止普通批准。请由本地安全操作者复现确认后，在受信终端使用 <code>guardctl approvals override-review-block</code>；浏览器会话不持有覆盖凭据。</span></div>}
        {item.status === "pending" && <div className="approval-actions"><label className="confirm-check"><input type="checkbox" checked={confirmed} onChange={e => setConfirmed(e.target.checked)} />我已核对工具、目标和参数摘要</label><div>{allowed.includes("deny") && <button className="button danger" disabled={mutation.isPending} onClick={() => { if (window.confirm("确认拒绝这次操作？该决定无法撤销。")) mutation.mutate("deny"); }}>拒绝</button>}{allowed.includes("allow-once") && <button className="button neutral" title={item.resolution_gate ? "复核门未解除" : undefined} disabled={!confirmed || mutation.isPending || !!item.resolution_gate} onClick={() => { if (window.confirm("确认仅允许这一次完全相同的操作？")) mutation.mutate("allow-once"); }}>允许一次</button>}</div>{mutation.error && <ErrorState error={mutation.error} />}</div>}
      </div>;
    })()}</Drawer>}
  </>;
}
