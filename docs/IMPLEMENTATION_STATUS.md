...
- Final local run: 69 Python tests passed; 1 symlink test was skipped because the Windows process lacks symlink creation permission. Nine TypeScript plugin tests and nine web-console tests passed; both TypeScript projects passed strict checking.
...
- The web console passes strict TypeScript compilation, a Vite production build, and Vitest component/helper tests. The latest production build is packaged under `guardd/ui/static/`, is about 441 KB gzip, uses route-level page chunks, and has no runtime CDN. Regenerate it with `npm run build` whenever `web/src/` changes.
